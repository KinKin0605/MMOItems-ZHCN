...
public class ElementsEdition extends EditionInventory {
            ...
            for (ElementStatType statType : ElementStatType.values()) {
                ...
                statLore.add(ChatColor.GRAY + "当前值: " + ChatColor.GREEN +
                        (statData.isPresent() && statData.get().hasStat(element, statType)
                                ...
                                : "---"));
                statLore.add("");
                statLore.add(ChatColor.YELLOW + AltChar.listDash + " 点击以更改此值.");
                statLore.add(ChatColor.YELLOW + AltChar.listDash + " 右键点击以删除此值.");
    ...
    public void whenClicked(InventoryClickEvent event) {
        ...
        if (event.getAction() == InventoryAction.PICKUP_ALL)
            ...
            player.sendMessage(MMOItems.plugin.getPrefix() + ChatColor.RED + edited.getKey().getName() + " " + edited.getValue().getName() + ChatColor.GRAY + " 成功移除.");
        }
    }
}
